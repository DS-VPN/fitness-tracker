...
<Modal bind:open title="Daily targets">
	...
	>
		...
		<div class="grid grid-cols-3 gap-3">
			<NumberField label="Protein" name="protein" bind:value={protein} min={0} step={1} suffix="g" required />
			<NumberField label="Carbs" name="carbs" bind:value={carbs} min={0} step={1} suffix="g" required />
			<NumberField label="Fat" name="fat" bind:value={fat} min={0} step={1} suffix="g" required />
		</div>
		{#if error}
